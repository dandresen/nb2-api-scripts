...
const argv = yargs 
    .options ({
        ...
        v: reqInput('vehicleId', 'vehicle id given by authority'),
...
var vehPredArchive = (agency, vehicle, start, end, max, environment) => {     
    ...
    var archiveUrl = {
        dev: `https://dev.api.nxbs2dev.com/v2.0/${agency}/vehicles/${vehicle}/predictions/archive?beginDate=${startUnix}&endDate=${endUnix}&max=${max}`,        
        stage: `https://staging.api.nxbs2dev.com/v2.0/${agency}/vehicles/${vehicle}/predictions/archive?beginDate=${startUnix}&endDate=${endUnix}&max=${max}`,        
        prod: `https://api.nextbus.com/v2.0/${agency}/vehicles/${vehicle}/predictions/archive?beginDate=${startUnix}&endDate=${endUnix}&max=${max}`,
    };
    ...
    var getVehPred = (options, vehicle, envName) => {
            ...
            if (!error && response.statusCode === 200) {
                ...
                jsonexport(finalArchiveObj, (err,csv) => {
                    if(err) {
                        ...
                        fs.writeFile(`./output/VehArchive-${agency}-Veh${vehicle}-${start}.csv`, csv, function(err) {
                            if(err) {
                                return console.log(err);
                            }
        
                            console.log(`Created output/VehArchive-${agency}-Veh${vehicle}-${start}.csv`);
                        });
                });
    ...
    if (environment == 'd'){
        ...
        getVehPred(options, vehicle, envName);
...
vehPredArchive(argv.a, argv.v, argv.s, argv.f, argv.m, argv.e);
